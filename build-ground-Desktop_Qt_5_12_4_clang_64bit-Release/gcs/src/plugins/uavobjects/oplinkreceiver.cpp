...
OPLinkReceiver::OPLinkReceiver(): UAVDataObject(OBJID, ISSINGLEINST, ISSETTINGS, NAME)
{
    ...
    fields.append(new UAVObjectField("LinkQuality", tr(""), "%", UAVObjectField::UINT8, LinkQualityElemNames, QStringList(), ""));
...
qint16 OPLinkReceiver::rssi() const
{
   QMutexLocker locker(mutex);
   return static_cast<qint16>(data_.RSSI);
}
void OPLinkReceiver::setRSSI(const qint16 value)
{
   mutex->lock();
...
quint16 OPLinkReceiver::linkQuality() const
{
   QMutexLocker locker(mutex);
   return static_cast<quint16>(data_.LinkQuality);
}
void OPLinkReceiver::setLinkQuality(const quint16 value)
{
   mutex->lock();
   bool changed = (data_.LinkQuality != static_cast<quint8>(value));
   data_.LinkQuality = static_cast<quint8>(value);
   mutex->unlock();
   if (changed) { emit linkQualityChanged(value); emit LinkQualityChanged(static_cast<quint8>(value)); }
}
