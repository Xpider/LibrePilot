...
AttitudeState::AttitudeState(): UAVDataObject(OBJID, ISSINGLEINST, ISSETTINGS, NAME)
{
    ...
    // NavYaw
    QStringList NavYawElemNames;
    NavYawElemNames << "0";
    fields.append(new UAVObjectField("NavYaw", tr(""), "degrees", UAVObjectField::FLOAT32, NavYawElemNames, QStringList(), ""));
...
void AttitudeState::emitNotifications()
{
    ...
    /*DEPRECATED*/ emit NavYawChanged(getNavYaw());
...
void AttitudeState::setNavYaw(const float value)
{
   ...
   mutex->unlock();
   if (changed) { emit navYawChanged(value); emit NavYawChanged(static_cast<float>(value)); }
}
