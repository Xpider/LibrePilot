...
StabilizationSettingsBank2::StabilizationSettingsBank2(): UAVDataObject(OBJID, ISSINGLEINST, ISSETTINGS, NAME)
{
    ...
    // AttitudeFeedForward
    QStringList AttitudeFeedForwardElemNames;
    AttitudeFeedForwardElemNames << "Roll" << "Pitch" << "Yaw";
    fields.append(new UAVObjectField("AttitudeFeedForward", tr(""), "sec", UAVObjectField::FLOAT32, AttitudeFeedForwardElemNames, QStringList(), "%BE:0:10; %BE:0:10; %BE:0:10;"));
    // RollRatePID
...
{
    emit attitudeFeedForwardRollChanged(attitudeFeedForwardRoll());
    /*DEPRECATED*/ emit AttitudeFeedForward_RollChanged(getAttitudeFeedForward_Roll());
    emit attitudeFeedForwardPitchChanged(attitudeFeedForwardPitch());
    /*DEPRECATED*/ emit AttitudeFeedForward_PitchChanged(getAttitudeFeedForward_Pitch());
    emit attitudeFeedForwardYawChanged(attitudeFeedForwardYaw());
    /*DEPRECATED*/ emit AttitudeFeedForward_YawChanged(getAttitudeFeedForward_Yaw());
...
float StabilizationSettingsBank2::attitudeFeedForward(quint32 index) const
{
   QMutexLocker locker(mutex);
   return static_cast<float>(data_.AttitudeFeedForward[index]);
}
void StabilizationSettingsBank2::setAttitudeFeedForward(quint32 index, const float value)
{
   mutex->lock();
   bool changed = (data_.AttitudeFeedForward[index] != static_cast<float>(value));
   data_.AttitudeFeedForward[index] = static_cast<float>(value);
   mutex->unlock();
   if (changed) { emit attitudeFeedForwardChanged(index, value); emit AttitudeFeedForwardChanged(index, static_cast<float>(value)); }
}

float StabilizationSettingsBank2::attitudeFeedForwardRoll() const { return attitudeFeedForward(0); }
void StabilizationSettingsBank2::setAttitudeFeedForwardRoll(const float value) { setAttitudeFeedForward(0, value); }
float StabilizationSettingsBank2::attitudeFeedForwardPitch() const { return attitudeFeedForward(1); }
void StabilizationSettingsBank2::setAttitudeFeedForwardPitch(const float value) { setAttitudeFeedForward(1, value); }
float StabilizationSettingsBank2::attitudeFeedForwardYaw() const { return attitudeFeedForward(2); }
void StabilizationSettingsBank2::setAttitudeFeedForwardYaw(const float value) { setAttitudeFeedForward(2, value); }
