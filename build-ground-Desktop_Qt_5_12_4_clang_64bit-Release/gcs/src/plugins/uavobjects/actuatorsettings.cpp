...
ActuatorSettings::ActuatorSettings(): UAVDataObject(OBJID, ISSINGLEINST, ISSETTINGS, NAME)
{
    ...
    // DShotMode
    QStringList DShotModeElemNames;
    DShotModeElemNames << "0";
    fields.append(new UAVObjectField("DShotMode", tr(""), "kHz", UAVObjectField::UINT16, DShotModeElemNames, QStringList(), "%BE:150:1200"));
    // ChannelMax
    ...
    BankModeEnumOptions << "PWM" << "PWMSync" << "OneShot125" << "OneShot42" << "MultiShot" << "DShot";
...
void ActuatorSettings::emitNotifications()
{
    ...
    emit dShotModeChanged(dShotMode());
    /*DEPRECATED*/ emit DShotModeChanged(getDShotMode());
...
quint16 ActuatorSettings::dShotMode() const
{
   QMutexLocker locker(mutex);
   return static_cast<quint16>(data_.DShotMode);
}
void ActuatorSettings::setDShotMode(const quint16 value)
{
   mutex->lock();
   bool changed = (data_.DShotMode != static_cast<quint16>(value));
   data_.DShotMode = static_cast<quint16>(value);
   mutex->unlock();
   if (changed) { emit dShotModeChanged(value); emit DShotModeChanged(static_cast<quint16>(value)); }
}
