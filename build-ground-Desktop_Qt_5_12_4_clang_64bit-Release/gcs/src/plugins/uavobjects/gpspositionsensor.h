...
class GPSPositionSensorConstants : public QObject {
    Q_OBJECT
public:
    enum Enum { StatusCount = 5, SensorTypeCount = 6, AutoConfigStatusCount = 4, BaudRateCount = 9,  };
...
class GPSPositionSensor_Status : public QObject {
    Q_OBJECT
public:
    enum Enum { NoGPS, NoFix, Fix2D, Fix3D, Fix3DDGNSS };
    Q_ENUMS(Enum) // TODO switch to Q_ENUM once on Qt 5.5
};
...
public:
    ...
    // VDOP
    // Status
    typedef enum { STATUS_NOGPS=0, STATUS_NOFIX=1, STATUS_FIX2D=2, STATUS_FIX3D=3, STATUS_FIX3DDGNSS=4 } StatusOptions;
    // Satellites
    // SensorType
    ...
    // Constants
    static const quint32 OBJID = 0xA529B6FE;
